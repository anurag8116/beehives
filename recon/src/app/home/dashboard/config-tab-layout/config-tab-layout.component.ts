...
import {TabDirective, TabsModule} from 'ngx-bootstrap';
...
export interface DialogData {
  parameters: any;
  name: any;
  requestFiltersStore: any;
}

@Component({
  selector: 'app-config-tab-layout',
  templateUrl: './config-tab-layout.component.html',
  styleUrls: ['./config-tab-layout.component.css']
})
export class ConfigTabLayoutComponent implements OnInit {
  ...
  public wgetMaterListDashlet = [];
  public dashboardParameters = [];
  public drillDowns = [];
  public showTab = false;
  public showContentBox = false;
    ...
    this.httpService.get('v1/dashboards/' + this.dashboardId + '?withConfig=' + true, true).subscribe(
      (data: any) => {
          ...
          if (item.dashboardLists) {
            item.dashboardLists.forEach((itemDeshboard, indexDashboard) => {
              if (item.id === this.dashboard.module.id && itemDeshboard.id === this.dashboard.id) {
                ...
                this.menuExpands[index] = {active: false};
              }
            });
          }
        });
        ...
        if (this.dashboardParameters) {
          ...
          this.dashboardParameters.length ? this.isFilterViewAble = true : this.isFilterViewAble = false;
        }
        this.drillDowns = data.drillDowns;
        if (this.dashboard) {
          if (this.dashboard.dashboardTabConfigs) {
            this.dashboard.dashboardTabConfigs = this.dashboard.dashboardTabConfigs.sort((a, b) => {
            ...
            this.dashboard.dashboardTabConfigs.forEach((item, index) => {
              let foundFlag = false;
              this.tabs.forEach((itemOne, indexOne) => {
                if (item.tabNumber === itemOne.tabNumber) {
                  foundFlag = true;
                }
              });
              if (!foundFlag) {
                this.tabs.push({
                  id: item.dashlet.id,
                  active: this.tabs.length === 0 ? true : false,
                  dashletType: item.dashlet.dashletType,
                  content: item.dashlet,
                  tabNumber: item.tabNumber,
                  customClass: '',
                  disabled: false,
                  removable: item.closable,
                  title: item.name
                });
              }
              this.wgetMaterListDashlet.push({
                'id': item.dashlet.id,
          ...
          if (this.dashboard.dashboardConfigs && this.dashboard.dashboardConfigs.length > 0) {
            this.dashboard.dashboardConfigs.forEach((item, index) => {
              let found = false;
              if (this.dashboard.dashboardTabConfigs) {
                this.dashboard.dashboardTabConfigs.forEach((itemOne, indexOne) => {
                  if (item.dashlet.id === itemOne.dashlet.id) {
                    found = true;
                  }
                });
              }
            ...
            if (this.dashboard.dashboardTabConfigs) {
              this.showTab = this.dashboard.dashboardTabConfigs.length > 0 ? true : false;
            }
            if (this.wgetMaterData.length > 0) {
            ...
            this.dashboard.dashboardDashlets.forEach((item, index) => {
              ...
              if (!found) {
                if ((this.wgetMaterData.length + 1) === (this.dashboard.dashboardDashlets.length - this.dashboard.dashboardTabConfigs.length) && ((this.dashboard.dashboardDashlets.length - this.dashboard.dashboardTabConfigs.length) % 2) !== 0) {
                  this.wgetMaterData.push({
                    'id': item.dashlet.id,
                    'type': item.dashlet.dashletType,
                    'config': {
  ...
  submit(): void {
    ...
    this.wgetMaterListDashlet.forEach((item, index) => {
      const config = {
        'sizey': item.config.sizey,
        'sizex': item.config.sizex,
        'unitx': item.config.unitx,
  ...
  public onTabChange(data: any) {
    if (data.dashletType !== 'LIST') {
      const myData = {dashletId: data.id, isTab: true};
      this.changetabValue.next(myData);
    }
  }
