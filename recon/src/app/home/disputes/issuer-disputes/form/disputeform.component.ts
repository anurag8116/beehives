...
@Component({
  selector: 'app-disputeform',
  templateUrl: './disputeform.component.html'
})
export class DisputeformComponent implements OnInit {
  ...
  constructor(private httpService: HttpService, private router: Router, private activatedRout: ActivatedRoute, private datePipe: DatePipe) {
  }

  ngOnInit() {
    this.getDisputeCategories();
    this.activatedRout.queryParams.subscribe(params => {
      this.disputeId = +params['disputeId'] || null;
      if (this.disputeId) {
        this.isEditMode = true;
      }
    });
    this.activatedRout.params.subscribe((param: Params) => {
      this.dataSourceId = +param['dataSourceId'] || null;
      this.transactionId = +param['transactionId'] || null;
      this.isArchive = param['isArchive'] || false;
      this.isReversalDispute = param['isReversalDispute'] ? JSON.parse(param['isReversalDispute']) : false;
      this.isDualDispute = param['isDualDispute'] ? JSON.parse(param['isDualDispute']) : false;
      this.disputeType = param['disputeType'] || null;
    });
  ...
  onRedirect() {
    if(!this.isEditMode) {
      this.router.navigate(['/home/disputes', 'issuer-disputes', {
        outlets: {
          'fullBodyOutlet': ['ntr-transaction']
        }
      }]);
    } else if (this.disputeType === null || this.disputeType === 'RAISED') {
      this.router.navigate(['/home/disputes', 'issuer-disputes', {
        outlets: {
          'fullBodyOutlet': ['list', this.stage ? this.stage : 'CHARGE_BACK']
    }
      }]);
    } else if (this.disputeType === 'ONUS') {
      this.router.navigate(['/home/disputes', 'onus', {
        outlets: {
          'fullBodyOutlet': ['list']
        }
      }]);
