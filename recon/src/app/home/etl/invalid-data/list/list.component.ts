  ...
  ngOnInit() {
    this.httpService.get('v1/invaliddataviews?start=0', true).subscribe(
      (data: any) => {
        ...
        this.showInvalidDataView(this.invalidDataViews[0].id);
      });

    this.select(0);
  }

  public showInvalidDataView(id: string): void {
    this.router.navigate(['/home/etl/part', 'invalid-data', {outlets: {'bodyOutlet': ['view', id]}}]);
  }

  public searchInvalidDataByName(id: string): void {
