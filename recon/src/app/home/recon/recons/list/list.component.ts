...
export class ListComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit() {
    this.dtOptions = this.dateTableService.getBasicTable('v1/recons', this.getColumnsDefinition());
  ...
  delete(id: number): void {
    this.httpService.delete('v1/recons/' + id, true).subscribe(
      (data: any) => {
        this.dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
          dtInstance.draw();
        });
        this.httpService.displaySuccessOnPopUp('Recon Successfully Deleted !');
      }
    );
  }

  edit(id: number): void {
    this.router.navigate(['/home/recon', 'recons', {outlets: {'fullBodyOutlet': ['new']}}], {queryParams: {id: id}});
  }
  ...
  private getColumnsDefinition(): any[] {
    ...
    columns.push({
      title: 'NAME', data: 'name', bSortable: false, render:  $.fn.dataTable.render.text()
    });
    columns.push({
  ...
  private onDetailButtonSelect() {
    ...
    jQuery('table').on('click', 'a#detailBtn', function () {
      ...
      that.router.navigate(['/home/recon', 'recons', {outlets: {'fullBodyOutlet': ['detail']}}], {queryParams: {id: jQuery(this).data('id')}});
    });
  }
}
